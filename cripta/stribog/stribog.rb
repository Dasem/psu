...
class Stribog
  ...
  def l_p(vector)
    ...
    result = Array.new
    for k in 0..7 do
      sub_vector = bits[(k*64) .. (k*64 + 63)]
      for i in 0..63 do
        temp = 0
        for j in 0..63 do
          temp += sub_vector[j] * @bits_matrix[i][j]
        end
        result << temp
      end
    end
  ...
  def compress(n, m, h)
    result = bytes_xor(h, n)
    result = l_p(p_p(s_p(result)))
  ...
  def e(k, m)
    result = bytes_xor(k, m)
    for i in 0..11
      result = l_p(p_p(s_p(result)))
      k = key_schedule(k, i)
      result = bytes_xor(result, k)
    end
  ...
  def to_bits(arr)
    ...
    for byte in arr do
      ("%08b" % byte).each_char { |j| result << j.to_i }
    end
  ...
  def bytes_to_int(bytes)
    to_int(to_bits(bytes))
  end

  def int_to_bytes(int)
    to_bytes(int_to_bits(int))
  end
  ...
  def to_bytes(arr)
    result = Array.new
    for i in 0..63 do 
      result << arr[i*8..(i*8+7)].join.to_i(2)
    end
  ...
  def to_64x64(arr)
    ...
    for bytes in arr do
      temp = Array.new
      ("%064b" % bytes).each_char { |j| temp << j.to_i }
      result << temp
    end
  ...
  def initialize
    ...
    @input = IO.read('message').bytes.to_a
...
7, 15, 23, 31, 39, 47, 55, 63]
    ...
    @a_matrix = [0x8e20faa72ba0b470, 0x47107ddd9b505a38, 0xad08b0e0c3282d1c, 0xd8045870ef14980e,
...
0x07e095624504536c, 0x8d70c431ac02a736, 0xc83862965601dd1b, 0x641c314b2b8ee083]
    ...
    @c = [[0xb1,0x08,0x5b,0xda,0x1e,0xca,0xda,0xe9,0xeb,0xcb,0x2f,0x81,0xc0,0x65,0x7c,0x1f,
...
0xfa,0xf4,0x17,0xd5,0xd9,0xb2,0x1b,0x99,0x48,0xbc,0x92,0x4a,0xf1,0x1b,0xd7,0x20]
  ...
  def hash
    ...
    message = @input
    while message.length >= 64 do
      m = message[-64 .. message.size]
      h = compress(n, message, h)
      n = int_to_bytes((bytes_to_int(n)+512)%pow)

      temp_int = bytes_to_int(s)+bytes_to_int(m)
      
      s = int_to_bytes(temp_int % pow)

      puts ((message.length / @input.length.to_f * 100).round(1).to_s + "% remaining")

      message = message[0..-65]
    end

    message = Array.new(63-message.length) {0} + [1] + message
    
    h = compress(n, m, h)
    
    n = int_to_bytes((bytes_to_int(n) + message.length*8) % pow)
    
    s = int_to_bytes((bytes_to_int(s) + bytes_to_int(m)) % pow)

    h = compress(empty_array, h, n)

    h = compress(empty_array, h, s)
  end
end

strbg = Stribog.new

puts strbg.hash.to_s
